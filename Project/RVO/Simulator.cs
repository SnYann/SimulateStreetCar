...
namespace RVO
{
    ...
    public class Simulator
    {
        ...
        private class Worker
        {
             ...
             */
            internal Worker(int start, int end, ManualResetEvent doneEvent,Simulator s)
            {
                start_ = start;
                end_ = end;
        ...
        //private Simulator instance_ = new Simulator();
        ...
        //song
        public int delAgentAt(int i)
        {
            if (i < 0 || i >= agents_.Count)
            {
                return -1;
            }
            agents_.RemoveAt(i);
        ...
        //song
        //public int delAgentID(int i)
        //{
        //    if (i < 0 || i >= agents_.Count)
        //    {
        //        return -1;
        //    }
        //    for (int ii = 0; ii < agents_.Count; ii++)
        //    {
        //        if (agents_[ii].id_ == i)
        //        {
        //            agents_.RemoveAt(i);
         ...
         * this agent.</param>
         */
        public int addAgent0(Vector2 position, float neighborDist, int maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed, Vector2 velocity)
        {
            Agent agent = new Agent(this);
            agent.id_ = agents_.Count;
        ...
        public float doStep()
        {
            ...
            if (workers_ == null)
            {
                ...
                for (int block = 0; block < workers_.Length; ++block)
                {
                    ...
                    workers_[block] = new Worker(block * getNumAgents() / workers_.Length, (block + 1) * getNumAgents() / workers_.Length, doneEvents_[block],this);
                }
            }
        ...
        {
            //song该方法已经舍弃用来感染新agent
            if (agentNo<agents_.Count && neighborNo < agents_[agentNo].agentNeighbors_.Count)
                return agents_[agentNo].agentNeighbors_[neighborNo].Value.id_;
            else
                return 1000;//随便设置了一个很大的数而已
