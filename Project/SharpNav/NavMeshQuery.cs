...
namespace SharpNav
{
    ...
    public class NavMeshQuery
    {
        ...
        public NavMeshQuery(TiledNavMesh nav, int maxNodes)
            : this(nav, maxNodes, new System.Random(Guid.NewGuid().GetHashCode()))// new Random()) song
        {
        }
        ...
        public void FindRandomPointOnPoly(NavPolyId polyId, out Vector3 randomPt)
        {
            ...
            if (!GetPolyHeight(polyId, randomPt, ref h))
                FindRandomPointOnPoly(polyId, out randomPt);
        ...
        //internal void FindRandomPoint(out Simulate.Vector2 posStart, out Simulate.Vector2 posEnd)
        //{
        ...
        public bool FindPath(ref NavPoint startPt, ref NavPoint endPt, NavQueryFilter filter, Path path)
        {
            ...
            if (startRef == endRef)
            {
                var temp = startPt.Position - endPt.Position;
                temp.Y = 0;
                path.SetCost(temp.Length());
                path.Add(startRef);
                return true;
            }
            ...
            path.SetCost(node.Cost);//song 最后一个节点的花费，放到path里的花费
            //Console.WriteLine("lastBestNode.Cost: "+lastBestNode.Cost);
            //Console.WriteLine("lastBestTotalCost: "+ lastBestTotalCost);
            ...
            while (node != null);
            //while (node != null||path.Count>200);//加了path数量限制
                ...
                for (int i = 0; i < path.Count; i++)
                {
                    ...
                    if (i + 1 < path.Count)
                    {
                        ...
                        if (GetPortalPoints(path[i], path[i + 1], ref left, ref right, ref fromType, ref toType) == false)
                        {
                            ...
                            stat = straightPath.AppendVertex(new StraightPathVertex(new NavPoint(path[i], FindRandomPointOnPoly(path[i])), StraightPathFlags.None));
        ...
        public bool FindCenterPath(Vector3 startPos, Vector3 endPos, Path path, StraightPath straightPath, PathBuildFlags options)
        {
            ...
            for (int i = 1; i < path.Count - 1; i++)
            {
                ...
                CenterPointOnPoly(path[i], ref centerPos);
                //centerPos = FindRandomPointOnPoly(path[i]);//song 随机取点而不是中心了
                straightPath.AppendVertex(new StraightPathVertex(new NavPoint(path[i], centerPos), StraightPathFlags.None));
            }
        ...
        public bool CenterPointOnPoly(NavPolyId reference, ref Vector3 closest)
        {
            NavTile tile;
            NavPoly poly;
            if (nav.TryGetTileAndPolyByRef(reference, out tile, out poly) == false)
            ...
            tile.centerPoint(poly, out closest);
            return true;
        }
        ...
        public NavPoint FindNearestPoly(Vector3 center, Vector3 extents)
        {
            ...
            result = this.FindNearestPoly(ref center, ref extents);
            return result;
        }
        ...
        public NavPoint FindNearestPoly(ref Vector3 center, ref Vector3 extents)
        {
            NavPoint nearestPt;
            ...
            if (!QueryPolygons(ref center, ref extents, polys))//song
